...
async function runScheduledJob() {
  ...
  const tickers = Object.values(get_all_tickers_resp).map(
    (item) => item.ticker,
  );
  ...
  const bullAppearDataWithValidDates = bullAppearData.filter(
    (item) =>
      item.analysis &&
  ...
  console.log(
    "Bull appear data with valid dates:",
    bullAppearDataWithValidDates.length,
  );
  const tickersToCalculate = tickers.filter(
    (ticker) =>
      !bullAppearDataWithValidDates.some((item) => item.ticker === ticker),
  );
  ...
  for (const ticker of tickersToCalculate) {
    ...
    while (attempts < maxAttempts) {
      try {
        ...
        attempts++;
        console.error(
          `Error fetching stock data for ${ticker}, attempt ${attempts} of ${maxAttempts}:`,
          error,
        );
        if (attempts < maxAttempts) {
          ...
          console.error(
            `Failed to fetch stock data for ${ticker} after ${maxAttempts} attempts.`,
          );
          return new Response(
            JSON.stringify({
    ...
    let analysisResultBullAppear = getAnalysisResults(
      bullAppearDates,
      stockData,
    );

    // write the dates to the cache db
    const { error } = await supabase.from("apex_bull_appear").insert({
      ticker: ticker,
      analysis: analysisResultBullAppear,
      latestClosePrice: stockData[stockData.length - 1]?.close.toFixed(2),
    });
    if (error) {
      console.error("Error fetching bull appear results:", error);
      return;
...
function getAnalysisResults(dates, stockData) {
  ...
  for (const date of dates) {
    ...
    if (dateIndex !== -1) {
      analysisResult[dateSimple].change1TD =
        stockData[dateIndex + 1] && stockData[dateIndex]
          ? ((stockData[dateIndex + 1].close - stockData[dateIndex].close) /
              stockData[dateIndex].close) *
            100
          : null;
      analysisResult[dateSimple].change5TD =
        stockData[dateIndex + 5] && stockData[dateIndex]
          ? ((stockData[dateIndex + 5].close - stockData[dateIndex].close) /
              stockData[dateIndex].close) *
            100
          : null;
      analysisResult[dateSimple].change20TD =
        stockData[dateIndex + 20] && stockData[dateIndex]
          ? ((stockData[dateIndex + 20].close - stockData[dateIndex].close) /
              stockData[dateIndex].close) *
            100
          : null;
