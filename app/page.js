      ...
      try {
        ...
        const response = await fetch("/api/analyze-stock", {
          ...
          body: JSON.stringify({
            ticker,
            settings,
            apexBullAppearDates: apexBullAppearDates[ticker],
          }),
        });
  ...
  return (
    <div className="container mx-auto p-4">
      <h1 className="text-4xl font-bold text-center mb-4">
        Optilens Stock Screener
      </h1>
      <h2 className="text-2xl text-center mb-8">
        Find stocks using technical indicators
      </h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <Card>
          <CardContent>
            ...
            {Object.entries(settings.indicatorSettings).map(
              ([indicator, { isEnabled }]) => (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <Input
                ...
                onChange={(e) =>
                  handleSettingChange("recency", parseInt(e.target.value))
                }
                min={1}
              />
            </div>
            <div>
              ...
              <Checkbox
                ...
                onCheckedChange={(checked) =>
                  handleSettingChange("showWinRate", checked)
                }
              />
            </div>
            {settings.showWinRate && (
              <div>
                ...
                <Input
                  ...
                  value={settings.x}
                  onChange={(e) =>
                    handleSettingChange("x", parseInt(e.target.value))
                  }
                  min={1}
                />
              </div>
            ...
            <div>
              ...
              <Checkbox
                ...
                onCheckedChange={(checked) =>
                  handleSettingChange("showOnlyIfAllSignalsMet", checked)
                }
              />
            </div>
          </div>
      ...
      {settings.showWinRate && screeningResults.length > 0 && (
        <div className="mt-4">
          <h3 className="text-lg font-semibold mb-2 text-center">
            Overall Win Rate (Stock rises {settings.x} days later)
          </h3>
          <p className="text-center">
            {(
              screeningResults.reduce(
                (acc, result) => acc + (result.winRate || 0),
                0,
              ...
              {screeningResults
                ...
                .map(({ ticker, dates, winRate }) => (
                  <TableRow key={ticker}>
                    ...
                    <TableCell>
                      {winRate !== undefined ? winRate.toFixed(2) : "N/A"}
                    </TableCell>
                  </TableRow>
                ))}
            </TableBody>
