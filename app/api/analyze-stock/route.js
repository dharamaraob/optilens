...
export async function POST(request) {
  ...
  if (settings.showWinRate) {
    ...
    let date_pool = dates.filter(
      (date) => new Date(date) < xCutoffDate && new Date(date) <= new Date(),
    );
    ...
    date_pool = date_pool.map((date) => {
      ...
      if (dateIndex !== -1 && data[dateIndex + settings.x]) {
        const increasedPrice =
          data[dateIndex + settings.x].close > data[dateIndex].close;
    ...
    // calculate win rate
    const winRate =
      (date_pool.filter((entry) => entry.increasedPrice).length /
        date_pool.length) *
      100;
