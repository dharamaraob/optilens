...
function findBearTraps(potentialTraps, fromDate, toDate) {
  ...
  for (const { date, low } of bearTrapsUpToDate) {
    ...
    const postBearTrapLows = bearTrapsUpToDate
      .filter(
        (trap) =>
          new Date(trap.date) > new Date(date) && new Date(trap.date) <= to,
      )
      .map((trap) => trap.low);
 ...
 */
function findLowestBearTrapWithinPriceRange(
  potentialTraps,
  upToDate,
  lowPrice,
  highPrice,
  ...
  const fromDate = new Date(
    new Date(upToDate).setFullYear(new Date(upToDate).getFullYear() - 1),
  );
  const bearTrapsUpToDate = findBearTraps(
    potentialTraps,
 ...
 */
function findHighestBullTrapWithinPriceRange(
  potentialTraps,
  upToDate,
  lowPrice,
  highPrice,
  ...
  const bullTrapsUpToDate = findBullTraps(
    potentialTraps,
    fromDate.toISOString().split("T")[0],
    upToDate,
  );
...
export {
  ...
  findHighestBullTrapWithinPriceRange,
};
