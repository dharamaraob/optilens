import {
  calculateSMA,
  get2DayAggregatedData,
  getLowInflexionPoints,
  getHighInflexionPoints,
  findBearTraps,
  findLowestBearTrapWithinPriceRange,
} from "./indicator_helpers.mjs";
function getApexBullAppearDates(data, showWinRate = true) {
  ...
  const condition = aggregatedData.map(
    (obj, idx) =>
      idx > 0 &&
      obj.High < aggregatedData[idx - 1].High &&
      obj.Low > aggregatedData[idx - 1].Low,
  );
  const wallabyDates = aggregatedData
    .map((obj) => obj.Date)
    .filter((_, idx) => condition[idx]);
  let bullAppearDates = [];
  ...
  for (let i = 0; i < wallabyDates.length; i++) {
    ...
    if (
      ...
      aggregatedData[kangarooPos].SMA_200 >
        aggregatedData[kangarooPos + 5].SMA_200
    ) {
    ...
    for (let i = 1; i < 5; i++) {
      ...
      if (
        ...
        if (
          (currData.Open >
            currData.Low + (4 / 5) * (currData.High - currData.Low) &&
            currData.Close >
              currData.Low + (4 / 5) * (currData.High - currData.Low)) ||
    ...
    for (let i = 0; i < 6; i++) {
      ...
      if (
        activeBearTraps.some(
          (trap) => trap[1] > currData.Low && trap[1] < currData.High,
        )
      ) {
        ...
        i > 0 &&
        ((currData.Low <= aggregatedData[currPos].SMA_20 &&
          aggregatedData[currPos].SMA_20 <= currData.High) ||
          (currData.Low <= aggregatedData[currPos].SMA_50 &&
            aggregatedData[currPos].SMA_50 <= currData.High) ||
          (currData.Low <= aggregatedData[currPos].SMA_200 &&
            aggregatedData[currPos].SMA_200 <= currData.High))
      ) {
...
function getApexBullRagingDates(data, showWinRate = true) {
  ...
  highInflexionPoints.forEach(
    ({ date: highPointDate, high: highPointValue }) => {
      if (!data.some((row) => row.date === highPointDate)) {
        return;
      }

      // Find the stopping point (which is the next bear trap)
      const stoppingPointDate =
        futureBearTraps.find(
          ([trapDate, trapValue]) =>
            trapDate > highPointDate && trapValue < highPointValue,
        )?.[0] || data[data.length - 1].date;

      futureBearTraps = futureBearTraps.filter(
        ([trapDate]) => trapDate >= highPointDate,
      );
      ...
      const midPoint =
        previousBearTrap[1] + (highPointValue - previousBearTrap[1]) / 2;

      // Analyze the range from high point to stopping point
      const rangeData = data.filter(
        (row) => row.date >= highPointDate && row.date <= stoppingPointDate,
      );
      ...
      const breakBelowBearTrap = rangeData.find(
        (row) => row.Low < previousBearTrap[1],
      );
      ...
      const postBreakData = data
        .filter((row) => row.date >= dateWhichBrokeBelowBearTrap)
        .slice(0, 6);
        ...
        if (
          ...
          (row.Close - row.Open > 0.5 * (row.High - row.Low) ||
            (row.Open > row.Low + (4 / 5) * (row.High - row.Low) &&
              row.Close > row.Low + (4 / 5) * (row.High - row.Low)))
        ) {
